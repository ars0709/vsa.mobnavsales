...
{
    [Activity(Label = "Edit User Device Profile")]
 
    public class NavUserDeviceProfileList : Activity
    {
        string[] items;
        IFungsiDB db = null;
        DataTable dt = null;
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            SetContentView(Resource.Layout.NavUserDeviceProfileList);
            if (db == null)
            {
        ...
        private void LoadData()
        {
            try
            {
                ListView lv = FindViewById<ListView>(Resource.Id.lstUser);
                dt = db.RetrieveData("select * from DeviceUser");
                if (dt != null && dt.Rows.Count > 0)
                {
                    NavUserDeviceProfilceAdapt ListAdapter = new NavUserDeviceProfilceAdapt(this, dt);
